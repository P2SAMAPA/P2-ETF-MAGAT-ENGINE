...
<style>
  ...
  .hero-conv   { font-size: 28px; font-weight: 500; color: #c2410c; margin-top: 6px; }
  .hero-date   { font-size: 15px; color: #6b7280; margin-top: 8px; }
  .hero-source { font-size: 14px; color: #9a3412; font-weight: 600;
                 background: #ffedd5; border-radius: 20px;
  ...
  .label-window { display:inline-block; font-size:14px; font-weight:700;
                  color:#c2410c; text-transform:uppercase; letter-spacing:.07em;
                  background:#ffedd5; border-radius:6px;
                  padding:5px 14px; margin-bottom:12px; }
  .window-badge { font-size:14px; color:#c2410c; background:#ffedd5;
...
def best_signal(sig_fixed: dict, sig_window: dict) -> tuple:
    ...
    if ret_w > ret_f and sig_window and "pick" in sig_window:
...
def main():
    ...
    st.markdown(
        ...
        "P2-ETF-MAGAT-ENGINE &nbsp;·&nbsp; Research only &nbsp;·&nbsp; Not financial advice"
        "</div>",
        unsafe_allow_html=True,
    )
